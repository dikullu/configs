...
local textclock = wibox.widget.textclock(
   markup(clockgf, space3 .. chosen_clock_type .. markup.font("Tamsyn 3", " ")))
...
local cpu = lain.widget.cpu({
    settings = function()
       widget:set_markup(
	  space3 .. cpu_now.usage .. "%" .. markup.font("Tamsyn 4", " "))
    end
})
...
kbdwidget:set_markup(
   "<span foreground=".."'"..beautiful.fg_normal.."'".."> DK </span>")
...
local fs = lain.widget.fs({
    ...
    settings = function()
	  ...
	  " %3.2f %s ", fs_now["/home"].free, fs_now["/home"].units)
...
local bat = lain.widget.bat({
    ...
    settings = function()
        ...
        if bat_now.status ~= "N/A" then
	       ...
	       widget:set_markup(
		  markup.font(
		     beautiful.font, markup.fg.color(
			beautiful.fg_normal, " +" .. bat_now.perc .. "%")))
	   ...
	   widget:set_markup(
	      markup.font(
		 beautiful.font, markup.fg.color(beautiful.fg_normal, " AC ")))
            bat_icon:set_image(beautiful.widget_ac)
        end
    end
})
...
globalkeys = awful.util.table.join(
   ...
   -- X screen locker
    ...
    awful.key({ altkey, "Control"}, "Right",
       function () lain.util.useless_gaps_resize(7) end,
    ...
    awful.key({                   }, "XF86Launch1",
       function()  awful.util.spawn("subl3") end),
    
    awful.key({ modkey            }, "v",
       function() awful.util.spawn_with_shell("vivaldi-snapshot") end ),

    awful.key({ modkey            }, "t",
       function() awful.util.spawn_with_shell("caja") end ),

    awful.key({ modkey            }, "r",
       function() awful.util.spawn('xterm -e ranger') end ),

    awful.key({                   }, "F11",
       function() awful.util.spawn('qpaeq') end ),

    awful.key({ modkey            }, "l",
       function() awful.util.spawn_with_shell("~/.config/scripts/lock.sh") end),

    awful.key({                   }, "Print",
       function() awful.util.spawn("scrot -e 'mv %f ~/pictures/screenshots/'") end),
    ...
    --awful.key({ }, "F4",
    --function () scratch.drop(
    --"weechat", "bottom", "left", 0.60, 0.60, true, mouse.screen) end),

    --awful.key({ }, "F6",
    --function () scratch.drop(
    --"smuxi-frontend-gnome", "bottom", "left", 0.60, 0.60, true, mouse.screen) end),

    awful.key({ }, "F2",function () scratch.drop(
	     "telegram-desktop", "bottom", "right", 0.50, 0.60, true, mouse.screen) end),
    
    awful.key({ }, "F3", function () scratch.drop(
	     "xterm -e ranger", "center", "center", 0.75, 0.7, true, mouse.screen) end),

    awful.key({ modkey }, "t", function() translate.show_translate_prompt() end,
    ...
    awful.key({ }, "XF86MonBrightnessUp",
       function () awful.util.spawn_with_shell("~/.backlight.sh up") end,
       {description = "+10%", group = "hotkeys"}),

    awful.key({ }, "XF86MonBrightnessDown",
       function () awful.util.spawn_with_shell("~/./.backlight.sh down") end,
       {description = "-10%", group = "hotkeys"}),
    ...
    --]]
    -- Prompt
    awful.key({ altkey }, "F2", function () awful.util.spawn(
	     "dmenu_run -fn 'Source Code Pro Regular-8' -i -l 10 -p 'Run:' -nb '#2d2d2d' -nf '#cccccc' -sb '#ff033e' -sf '#38000d'") end),
...
awful.rules.rules = {
    ...
    { rule = { class = "Caja" },
      properties = {
	 floating = true, geometry = { x=200, y=150, height=600, width=1100 } } },
